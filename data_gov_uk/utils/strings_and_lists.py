...
class StringOperations:
    ...
    def _convert_string_to_numbers(self, d_type: str, notations: dict[str, int] | None = None) -> int | float:
        ...
        try:
            ...
            raise ConversionError(f"'{number}' could not be converted to float") from exc

        for end in notations:
    ...
    def convert_to_integer(self) -> int:
...
class ClassInitiationError(Exception):
    pass


# Deprecated alias — kept for backward compatibility.
ClassIntiationError = ClassInitiationError
...
class ListOperations:
    ...
    def __init__(self, search_list: list, **kwargs) -> None:
    ...
    def _get_matching_scores_for_string(self) -> list[float]:
    ...
    def get_best_matching_string(self) -> str:
        ...
        Returns:
            str: The string from the list with the highest similarity ratio
                to search_string.  Returns None if the list is empty.

        Note:
            Similarity is determined using the ``SequenceMatcher`` from ``difflib``,
            comparing lowercased versions of the strings.  Returns only the *best*
            match, not a list of matches ranked by similarity.
        """
        matching_ratios = self._get_matching_scores_for_string()
        return self.search_list[matching_ratios.index(max(matching_ratios))]

    def search_list_by_snowball(self) -> list[str] | None:
        """Searches a list of strings using the Snowball stemmer.

        Filters a list of strings, retaining only those that contain the stemmed
        version of a given search string.  Uses the Snowball stemmer for stemming both
        the search string and the strings in the list.

        Returns:
            A list containing the strings from ``search_list`` that contain the stemmed
            ``search_string``, or None if no matches are found.
        """
        filtered = [
            x for x in self.search_list if x and _get_stemmer().stem(self.search_string) in _get_stemmer().stem(x)
        ]
        if filtered:
            return filtered
        else:
    ...
    def search_list_by_string_for_metric(self, search_metric: float | str) -> list | None:
        ...
        Args:
            search_metric (float | str): The metric to use for filtering.
                Can be a float, "mean", "median", "0.25", "0.75", or "0.5".
        ...
        Raises:
            None.  Handles invalid string inputs gracefully by returning None.
            Errors in ``_get_matching_scores_for_string`` are not explicitly handled here.
        """
        ...
        if search_metric in str_metrics:
            ...
            if isinstance(search_metric, float):
                score_indexes = [x >= search_metric for x in matching_scores]

        filtered = [self.search_list[x] for x in list(itertools.compress(range(len(score_indexes)), score_indexes))]
        if filtered:
            return filtered
        else:
    ...
    def get_unique_sorted_elements_by_key(self, id_key: str, sort_key: str | None = None) -> list[dict]:
        """
        ...
        if self.search_list:
            for key in [id_key, sort_key]:
                if key and key not in self.search_list[0]:
    ...
    def get_single_result_dict(self) -> dict:
        """
